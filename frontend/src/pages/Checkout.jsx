...
export default function Checkout() {
  ...
  const RadioDot = ({ active }) => (
    <span className={`flex-shrink-0 w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${active ? "border-indigo-500 bg-indigo-500" : "border-slate-300"
      }`}>
      {active && (
    ...
    <>
      ...
      <div className="min-h-screen bg-slate-50">
        ...
        <div className="border-b border-slate-200 bg-white">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 py-3 flex items-center gap-2 text-sm text-slate-500">
            ...
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
            <span className="text-slate-900 font-medium">{t('checkout')}</span>
          </div>
        </div>
              ...
              {[
                ...
                <React.Fragment key={i}>
                  <li className="flex items-center gap-2">
                    <span className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${step.done ? "bg-indigo-600 text-white" : step.active ? "bg-indigo-600 text-white ring-4 ring-indigo-100" : "bg-slate-200 text-slate-500"
                      }`}>
                      {step.done ? (
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>
                      ) : i + 1}
                    </span>
                    <span className={`text-sm font-semibold ${step.active ? "text-indigo-600" : step.done ? "text-slate-700" : "text-slate-400"}`}>{step.label}</span>
                  </li>
                  {i < arr.length - 1 && (
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
              <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                ...
                {user && (
                  <Link to="/profile?tab=addresses" className="text-xs text-indigo-600 hover:underline font-semibold">
              ...
              <div className="p-6">
                {!user ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="mt-2 text-sm text-slate-500">{t('noSavedAddresses') || "No saved addresses."}</p>
                        ...
                        >
                          ...
                          <div className="min-w-0">
                            <div className="flex items-center gap-2">
                              ...
                              {addr.is_default && (
                                <span className="text-[10px] bg-indigo-100 text-indigo-700 rounded-full px-2 py-0.5 font-semibold uppercase tracking-wide">Default</span>
                              )}
                            </div>
              ...
              <div className="p-6 space-y-4">
                  ...
                  <div className="flex-1 p-4 flex items-center gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-bold text-slate-900">{t('paymentMethodBakong') || "KHQR / Bakong"}</span>
                        <span className="text-[10px] bg-emerald-100 text-emerald-700 rounded-full px-2 py-0.5 font-semibold uppercase tracking-wide">Instant</span>
                      </div>
                      <p className="text-xs text-slate-500 mt-0.5">{t('paymentMethodBakongDesc') || "Scan QR with any Cambodian bank app"}</p>
                ...
                <label className={`group flex gap-0 rounded-2xl border-2 overflow-hidden cursor-pointer transition-all ${paymentMethod === "card_visa"
                  ...
                  <div className="flex-1 p-4 flex items-center gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-bold text-slate-900">{t('paymentMethodCardVisa') || "Credit / Debit Card"}</span>
                        <span className="text-[10px] bg-slate-100 text-slate-600 rounded-full px-2 py-0.5 font-semibold uppercase tracking-wide">Secure</span>
                      </div>
                      <p className="text-xs text-slate-500 mt-0.5">{t('paymentMethodCardDesc') || "Visa, Mastercard, Amex, UnionPay"}</p>
                      <div className="flex items-center gap-1.5 mt-2.5">
                        {/* Visa */}
            ...
            <div className="lg:hidden">
              <button
                ...
                className={`w-full rounded-2xl text-white py-4 text-base font-bold shadow-lg hover:opacity-90 active:scale-95 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed ${paymentMethod === "bakong_khqr" ? "bg-[#C88F09] shadow-amber-200" : "bg-indigo-600 shadow-indigo-200"
                  }`}
              >
                {loading ? (
              ...
              <Link to="/cart" className="mt-3 w-full rounded-2xl bg-white border border-slate-200 text-slate-700 py-3.5 text-sm font-semibold hover:bg-slate-50 transition-colors flex items-center justify-center gap-2">
